...
from collectors.center.tools.ibmc.data_proc import *
...
class IBMCCollector(Collector):
    ...
    def start_collection(self, interval=1):
        ...
        os.system("rm -rf data/[0-9]*")
        if not os.path.exists("data/pid"):
            os.system("mkdir -p data/pid")
        self.end_collection()
        for ip in self.ibmc_dict:
            os.system(f"nice -n -20 python ibmc_data_collect.py -i \"{self.ibmc_dict[ip]}\" -s \"{self.collected_sets}\" \
                -l {interval} -t {MAX_COLLECTED_TIME} -a {ip}&")
        os.chdir(cur_dir)
    ...
    def power_proc(self, collect_time):
        for ip in os.listdir("./data"):
            ...
            time_info = collected_time_cal(collect_time, ip)
            power_info = power_cal(collect_time, ip)
            if len(time_info) == 0:
                LOGGER.write(LOGGER.global_log, f"[IBMCCollector]: {ip} fails to collect power.")
                continue
            result = [time_info[i]+power_info[i] for i in range(len(time_info))]
            result = pd.DataFrame(result)
            result.columns = ["time"] + self.avail_sets["power"][1:]
            self.data[ip] = result
